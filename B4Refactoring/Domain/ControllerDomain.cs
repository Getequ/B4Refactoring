...
namespace Getequ.B4Refactoring.Domain
{
    ...
    public class ControllerDomain
    {
        ...
        public ControllerDomain(Project project)
        {
            _project = project;
        }

        public IEnumerable<CodeClass2> FindControllers(bool thisProject = true)
        {
            ...
            if (thisProject)
            {
                if (_project.Object is VSProject)
                    _project.GetProjectClassList(_classList, _interfaceList, _project.DefaultNamespace(), new List<string> { "Controller", "ViewModel", "Service", "Module" });
                else
                    throw new ArgumentException(_project.Name.Q("'") + " не является проектом C#");
            }

            _project.GetClassList(null, _platformInterfaceList, "Bars.B4", null);

            return _classList.Where(x => x.Key.EndsWith("Controller")).Select(x => x.Value).ToList();
        }

        /// <summary>Найти для контроллера сервис, вью-модель и класс модуля (для регистрации создаваемых объектов)</summary>
        public void FindControllerRelativeClasses(string entityName)
        {
            if (_project.Object is VSProject)
                _project.GetProjectClassList(_classList, _interfaceList, _project.DefaultNamespace(), 
                    new List<string> { 
        ...
        /// <summary>Найти контроллеры с проблемными местами</summary>
        public IEnumerable<ControllerStinck> FindStinck(IEnumerable<CodeClass2> controllers)
        {
            ...
            foreach (CodeClass2 ctrl in controllers)
            {
                ...
                var stinck = new ControllerStinck{ Name = ctl.Name.CutLast("Controller"), Class = ctl };
        ...
        public ClassGenerationResult GenerateRefactoringCode(ControllerStinck ctl)
        {
            ...
            CheckViewModelGetSelfReference(ctl, vmClass != null ? new CodeClassWrapper(vmClass) : null, ctl.BadPublicMethods.FirstOrDefault(x => x.Name == "Get"));
        ...
        private void CheckViewModelGetSelfReference(ControllerStinck ctl, CodeClassWrapper vmClass, CodeFunctionWrapper get)
        {
            if (get == null) return;

            // Если ВМ нет, либо она не содержит перегрузки для Гет - заменить вызов ВМ на ДС
            if (vmClass == null || !vmClass.Methods.Any(m => m.IsOverride && m.Name == "Get"))
            {
                for (int i = 0; i < get.Code.Count; i++)
                {
                    if (get.Code[i].Contains("ViewModel.Get(DomainService, baseParams).Data"))
                    {
                        var getCall = get.Code[i].Substring(get.Code[i].IndexOf("ViewModel.Get("));
                        getCall = getCall.Substring(0, getCall.IndexOf(';'));
                        get.Code[i] = get.Code[i].Replace(getCall, "domainService.Get(baseParams.Params[\"id\"].To<long>())");
                    }
                }
            }
        }
        ...
        private Dictionary<string, string> ExtractIoCDependencies(ControllerStinck stinck, IList<string> fileCode, IEnumerable<MethodConvertResult> methods, List<string> knownTypes)
        {
            ...
            {
                var generics = method.Method.GenericTypes;

                for (int i = 0; i < method.Code.Count; i++)
                {
                    var line = method.Code[i];
                    // not to refactor generic type resolvings
                    if (generics.Any(g => line.Contains("<" + g + ">")))
                        continue;
            ...
            ControllerModification modification = null)
        {
            ...
            {
                result.ClassCode = UpdateClassCode(stinck, baseName, @class, methods, interfaceName, modification, ci);
            }
        ...
        private string UpdateClassCode(ControllerStinck stinck, string baseClass, CodeClass2 @class, IEnumerable<CodeFunctionWrapper> methods, 
            string interfaceName, ControllerModification modification, ControllerConvertInfo ci)
        {
            ...
            if (ci.deps.Any())
            {
                ...
                {
                    if (wrapped.Properties.Any(x => x.Code.ContainsString(" " + dep.Value + " ")))
                        continue;
            ...
            // Добавить базовый класс/интерфейс
            if (ci.specificMethods.Any(x => x.IsOverride) && string.IsNullOrWhiteSpace(wrapped.Base) && !string.IsNullOrWhiteSpace(baseClass))
            {
                var defLine = code.First(l => l.Contains(wrapped.Name));
                var defIndex = code.IndexOf(defLine);

                code[defIndex] = defLine.Replace(" : ", " : " + baseClass + ", ");
            }

            if (ci.specificMethods.Any(x => !x.IsOverride) && !string.IsNullOrWhiteSpace(interfaceName) && !wrapped.Interfaces.Any(i => i.Name == interfaceName))
            {
                var defLine = code.First(l => l.Contains(wrapped.Name));
                var defIndex = code.IndexOf(defLine);

                code[defIndex] = defLine + ", " + interfaceName;
            }

            return string.Join(Environment.NewLine, code);
        }

        private string GenerateClassCode(ControllerStinck stinck, string baseName, string classType, string subSpace, 
            string interfaceName, ControllerModification modification, ControllerConvertInfo ci, IList<string> fileCode)
        {
            var code = new List<string>();

            if (!ci.specificMethods.Union(ci.otherMethods).Any(m => m.Code.ContainsAnyWord(new []{ "ActionResult", "FileStreamResult" }) ))
            {
                ci.outerUsings.Remove("using System.Web.Mvc;");
                ci.innerUsings.Remove("    using System.Web.Mvc;");
            }
        ...
        /// <summary>Сгенерировать код интерфейса</summary>
        ...
        private string GenerateInterfaceCode(string interfaceName, string subSpace, IEnumerable<MethodConvertResult> methods)
        {
            ...
            var paramTypes = methods.SelectMany(x => x.Method.Parameters.Select(p => p.Type.AsFullName)).Distinct().ToList();
            var spaces = new List<string>();
            foreach (var type in paramTypes)
            {
                spaces.Add(type.Substring(0, type.LastIndexOf('.')));
            }
            spaces = spaces.Distinct().ToList();

            var orderedSpaces = new List<string>();

            orderedSpaces.AddRange(spaces.Where(x => x.StartsWith("System")));

            if (orderedSpaces.Any())
                orderedSpaces.Add("");

            spaces = spaces.Where(x => !x.StartsWith("System")).ToList();

            orderedSpaces.Add("B4");
            orderedSpaces.AddRange(spaces.Where(x => x.StartsWith("Bars.B4.")).Select(x => x.CutFirst("Bars.")));
            spaces = spaces.Where(x => !x.StartsWith("Bars.B4")).ToList();

            if (orderedSpaces.Any())
                orderedSpaces.Add("");

            orderedSpaces.AddRange(spaces.Where(x => !x.StartsWith("Bars.B4.") && !x.StartsWith(_project.DefaultNamespace())).Select(x => x.CutFirst("Bars.")));


            orderedSpaces.Add("Entities");

            orderedSpaces.AddRange(spaces.Where(x => x.StartsWith(_project.DefaultNamespace()) && !(x.CutFirst(_project.DefaultNamespace() + ".") == "Entities")).Select(x => x.CutFirst(_project.DefaultNamespace() + ".")));

            orderedSpaces.ForEach(ns.InnerUsing.Add);
        ...
        /// <summary>Добавить в интерфейс новые сигнатуры</summary>
        ...
        private MethodConvertResult ConvertMethod(ControllerStinck stinck, IList<string> fileCode, CodeFunctionWrapper method, string propName, CodeInterface platform)
        {
            ...
            var genericTypes = method.GenericTypes;
            ...
            #region check domainService required inclusion to signature
            ...
            #region convert Controller to B4.Core components
            ...
            if (!body.ContainsWord("ContentType"))
            {
                wordPairs.Add("JsonNetResult", "BaseDataResult");
            }
            ...
            var stringPairs = new Dictionary<string, string>
            {
                ...
                {"JsSuccess();",            "new BaseDataResult();"},
                {"JsonListResult.EmptyList","new ListDataResult()"},
                {"JsonNetResult.Success;",  "new BaseDataResult();"},
                {"JsonNetResult.Failure",   "BaseDataResult.Error"},
                {"JsFailure",               "BaseDataResult.Error"},
                {"Js(",                     "new BaseDataResult("},
                {"JsList",                  "new ListDataResult"}
            };

            Func<string, string, string> addPrivateDS = (s, p) =>
            {
                if (s.Contains(p + "("))
                {
                    return s.Replace(p + "(", p + "(domainService, ");
                }

                if (s.Contains(p + "<"))
                {
                    var start = s.IndexOf(p + "<");
                    var end = s.Substring(start).IndexOf(">")+1;

                    return s.Replace(s.Substring(start, end) + "(", s.Substring(start, end) + "(domainService, ");
                }
            ...
            foreach (var prive in dsRequiredPrivates)
            {
                for(int i = 0; i < body.Count; i++)
                    body[i] = addPrivateDS(body[i], prive);
            }

            body = body.Replace(stringPairs).ReplaceWords(wordPairs).ToList();

            // не заменяется для методов, возвращающих файлы
        ...
        /// <summary>Сгенерировать код, которым будет заменён имеющийся в методе код</summary>
        ...
        /// <summary>Требуется ли включить ДС в параметры метода, так как код метода работает с домен-сервисом</summary>
        ...
        /// <summary>Вычистить из контроллера всё, что выносится в отдельные классы</summary>
        ...
        private string ClearController(CodeClassWrapper cls, ControllerModification mod)
        {
            ...
            var propLine = clsProps.Any() ? clsProps.Max(x => x.EndLine) : 0;
            if (propLine == 0)
            {
                var method = clsMethods.OrderBy(x => x.StartLine).First();
                propLine = method.StartLine - 1 - method.DocComment.Untag("doc").Trim().Split('\n').Count();
            }

            // collect used in remain members
            foreach (var member in clsMethods.Where(x => !mod.ConvertedMethods.Any(y => x == y.Method)))
            {
                ...
                foreach (var method in clsMethods.Where(x => !mod.ConvertedMethods.Any(y => x == y.Method) && x != member && !x.IsOverride))
                {
            ...
            {
                if (leftMethods.Any(x => x.Code.Any(y => y.ContainsWord(method.Name) && !method.IsOverride)))
                    continue;
        ...
        private void AddModuleDependencies(IEnumerable<Registration> deps)
        {
            ...
            var lastUsing = module.FileCode.Last(x => x.StartsWith("    using "));
            var lastIndex = module.FileCode.IndexOf(lastUsing);

            foreach (var missed in missedUsings.Distinct().ToList())
            {
                module.FileCode.Insert(lastIndex+1, missed);
            }
